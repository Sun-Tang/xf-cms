...
const alterGoodsInfo = data => {
    return http.put("/indexConfigs", { configType: 3, ...data });
};
// 新增热销商品信息
const goodsInsert = data => {
    return http.post("/indexConfigs", { configType: 3, ...data });
};
/* 新品上线 */
// 获取新增商品配置列表
...
const alterNewGoodsInfo = data => {
    return http.put("/indexConfigs", { configType: 4, ...data });
};
// 新增商品信息
const newGoodsInsert = data => {
    return http.post("/indexConfigs", { configType: 4, ...data });
};
/* 为你推荐 */
// 获取配置列表
...
const alterRecommendGoodsInfo = data => {
    return http.put("/indexConfigs", { configType: 5, ...data });
};
// 新增商品信息
const recommendGoodsInsert = data => {
    return http.post("/indexConfigs", { configType: 5, ...data });
};
export default {
    // #region
    // 轮播图
