...
<script setup>
    ...
    const query = async () => {
        ...
        try {
            ...
            } = await api.home.getNewGoodsList(pageNumber, pageSize);
            if (+resultCode !== 200) {
                tableData.tableList = [];
    ...
    const remove = async (ids) => {
        try {
            let { resultCode } = await api.home.delNewGoodsInfo(ids);
            if (+resultCode !== 0) {
    ...
    const handleFormData = async (formDataObj, cancel) => {
        ...
        try {
            // 新增
            let sendfunc = api.home.newGoodsInsert,
                // 提交到后台的表单数据
                data = { ...popForm.ruleForm, ...formDataObj };
            // 修改
            let configId = popForm.configId;
            if (configId) {
                sendfunc = api.home.alterNewGoodsInfo;
