...
<script setup>
    ...
    // 商品编号校验：必须为5位数字
    const checkGoodsId = (_, value, callback) => {
        let reg = /^\d{5}$/;
        if (!reg.test(value)) {
            callback(new Error("商品编号必须为5位数字"));
            return;
        }
        callback();
    };
    ...
    const openDialog = (row) => {
        ...
        if (row) {
            let { configId, configName, redirectUrl, goodsId, configRank } = row;
            popForm.ruleForm = {
                configName,
                redirectUrl,
                goodsId,
    ...
    const query = async () => {
        ...
        try {
            ...
            } = await api.home.getRecommendGoodsList(pageNumber, pageSize);
            if (+resultCode !== 200) {
                tableData.tableList = [];
    ...
    const remove = async (ids) => {
        try {
            let { resultCode } = await api.home.delRecommendGoodsInfo(ids);
            if (+resultCode !== 0) {
    ...
    const handleFormData = async (formDataObj, cancel) => {
        ...
        try {
            // 新增
            let sendfunc = api.home.recommendGoodsInsert,
                // 提交到后台的表单数据
                data = { ...popForm.ruleForm, ...formDataObj };
            // 修改
            let configId = popForm.configId;
            if (configId) {
                sendfunc = api.home.alterRecommendGoodsInfo;
                data.configId = configId;
            }
            // 新增 / 修改热销商品信息;
            let { resultCode } = await sendfunc(data);
            if (+resultCode !== 200) {
                ElMessage.error(`${configId ? "修改" : "新增"}商品失败, 请重试`);
                return;
            }
            ElMessage.success(`${configId ? "修改" : "新增"}商品成功`);
            // 再次初始化popForm.configId
            popForm.configId = "";
            //新增商品，重新渲染第1页；修改商品，重新渲染当前页
            paginationData.pageNumber = configId ? paginationData.pageNumber : 1;
            // 重新获取数据
            query();
            // 关闭弹框
            cancel();
